package com.uxicons.compose.uicons.solid.chubby

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _DiagramPrevious: ImageVector? = null

val Icons.Sc.DiagramPrevious: ImageVector
    get() = _DiagramPrevious ?: UXIcon(name = "DiagramPrevious") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(22.66f, 13.31f)
                curveToRelative(0.19f, 0.2f, 0.29f, 0.47f, 0.27f, 0.74f)
                curveToRelative(-0.22f, 3.75f, -0.93f, 7.02f, -0.96f, 7.16f)
                curveToRelative(-0.07f, 0.34f, -0.33f, 0.62f, -0.66f, 0.73f)
                curveToRelative(-0.13f, 0.04f, -3.23f, 1.05f, -9.32f, 1.05f)
                reflectiveCurveToRelative(-9.19f, -1.01f, -9.32f, -1.05f)
                curveToRelative(-0.33f, -0.11f, -0.58f, -0.39f, -0.66f, -0.73f)
                curveToRelative(-0.03f, -0.14f, -0.74f, -3.41f, -0.96f, -7.16f)
                curveToRelative(-0.04f, -0.56f, 0.44f, -1.07f, 1.0f, -1.06f)
                horizontalLineToRelative(8.4f)
                verticalLineToRelative(-2f)
                horizontalLineToRelative(-1.5f)
                curveToRelative(-0.8f, 0.03f, -1.3f, -0.98f, -0.8f, -1.6f)
                curveToRelative(0.91f, -1.22f, 1.99f, -2.29f, 3.2f, -3.2f)
                curveToRelative(0.35f, -0.27f, 0.84f, -0.27f, 1.2f, 0f)
                curveToRelative(1.21f, 0.91f, 2.29f, 1.98f, 3.2f, 3.2f)
                curveToRelative(0.5f, 0.62f, 0f, 1.63f, -0.8f, 1.6f)
                horizontalLineToRelative(-1.5f)
                verticalLineToRelative(2f)
                horizontalLineToRelative(8.47f)
                curveToRelative(0.28f, 0f, 0.54f, 0.11f, 0.73f, 0.31f)
                close()
                moveTo(1.4f, 10.69f)
                curveToRelative(-0.19f, -0.2f, -0.29f, -0.47f, -0.27f, -0.74f)
                curveToRelative(0.22f, -3.75f, 0.93f, -7.02f, 0.96f, -7.16f)
                curveToRelative(0.08f, -0.34f, 0.33f, -0.62f, 0.66f, -0.73f)
                curveToRelative(0.13f, -0.04f, 3.22f, -1.05f, 9.32f, -1.05f)
                reflectiveCurveToRelative(9.19f, 1.01f, 9.32f, 1.05f)
                curveToRelative(0.33f, 0.11f, 0.58f, 0.39f, 0.66f, 0.73f)
                curveToRelative(0.03f, 0.14f, 0.74f, 3.41f, 0.96f, 7.16f)
                curveToRelative(0.04f, 0.56f, -0.44f, 1.07f, -1.0f, 1.06f)
                horizontalLineToRelative(-4.21f)
                curveToRelative(0.22f, -0.62f, 0.22f, -1.27f, 0.03f, -1.88f)
                lineToRelative(2.8f, -2.8f)
                curveToRelative(-0.14f, -1.03f, -0.29f, -1.88f, -0.4f, -2.43f)
                lineToRelative(-3.51f, 3.51f)
                curveToRelative(-0.64f, -0.75f, -1.33f, -1.45f, -2.08f, -2.09f)
                lineToRelative(2.07f, -2.07f)
                curveToRelative(-0.77f, -0.08f, -1.65f, -0.15f, -2.64f, -0.19f)
                lineToRelative(-1.12f, 1.12f)
                curveToRelative(-0.92f, -0.32f, -1.97f, -0.17f, -2.78f, 0.43f)
                curveToRelative(-1.37f, 1.02f, -2.58f, 2.23f, -3.6f, 3.6f)
                curveToRelative(-0.61f, 0.81f, -0.76f, 1.86f, -0.43f, 2.8f)
                horizontalLineToRelative(-4.01f)
                curveToRelative(-0.28f, 0f, -0.54f, -0.11f, -0.73f, -0.31f)
                close()
                moveTo(3.2f, 9f)
                horizontalLineToRelative(0.99f)
                lineToRelative(5.96f, -5.96f)
                curveToRelative(-1.17f, 0.04f, -2.19f, 0.13f, -3.06f, 0.23f)
                lineToRelative(-3.68f, 3.68f)
                curveToRelative(-0.08f, 0.64f, -0.16f, 1.33f, -0.22f, 2.06f)
                close()
            }
        }.also { _DiagramPrevious = it}
