package com.uxicons.compose.uicons.solid.chubby

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _SlidersVSquare: ImageVector? = null

val Icons.Sc.SlidersVSquare: ImageVector
    get() = _SlidersVSquare ?: UXIcon(name = "SlidersVSquare") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(21.98f, 2.79f)
                curveToRelative(-0.08f, -0.38f, -0.38f, -0.68f, -0.76f, -0.76f)
                curveToRelative(-0.19f, -0.04f, -4.71f, -1.02f, -9.21f, -1.02f)
                reflectiveCurveToRelative(-9.03f, 0.98f, -9.21f, 1.02f)
                curveToRelative(-0.38f, 0.08f, -0.68f, 0.38f, -0.76f, 0.76f)
                curveToRelative(-0.04f, 0.19f, -1.02f, 4.71f, -1.02f, 9.21f)
                reflectiveCurveToRelative(0.98f, 9.02f, 1.02f, 9.21f)
                curveToRelative(0.08f, 0.38f, 0.38f, 0.68f, 0.76f, 0.76f)
                curveToRelative(0.19f, 0.04f, 4.71f, 1.02f, 9.21f, 1.02f)
                reflectiveCurveToRelative(9.02f, -0.98f, 9.21f, -1.02f)
                curveToRelative(0.38f, -0.08f, 0.68f, -0.38f, 0.76f, -0.76f)
                curveToRelative(0.04f, -0.19f, 1.02f, -4.71f, 1.02f, -9.21f)
                reflectiveCurveToRelative(-0.98f, -9.02f, -1.02f, -9.21f)
                close()
                moveTo(10f, 10f)
                horizontalLineToRelative(-0.93f)
                curveToRelative(-0.04f, 0.7f, -0.07f, 1.37f, -0.07f, 2f)
                curveToRelative(0f, 2.19f, 0.25f, 4.39f, 0.46f, 5.86f)
                curveToRelative(0.08f, 0.55f, -0.3f, 1.05f, -0.85f, 1.13f)
                curveToRelative(-0.54f, 0.08f, -1.06f, -0.3f, -1.13f, -0.85f)
                curveToRelative(-0.22f, -1.53f, -0.48f, -3.84f, -0.48f, -6.14f)
                curveToRelative(0f, -0.64f, 0.03f, -1.31f, 0.07f, -2f)
                horizontalLineToRelative(-1.07f)
                curveToRelative(-0.55f, 0f, -1f, -0.45f, -1f, -1f)
                reflectiveCurveToRelative(0.45f, -1f, 1f, -1f)
                horizontalLineToRelative(1.23f)
                curveToRelative(0.07f, -0.7f, 0.15f, -1.41f, 0.26f, -2.14f)
                curveToRelative(0.08f, -0.55f, 0.59f, -0.92f, 1.13f, -0.85f)
                curveToRelative(0.55f, 0.08f, 0.93f, 0.59f, 0.85f, 1.13f)
                curveToRelative(-0.09f, 0.63f, -0.17f, 1.25f, -0.23f, 1.86f)
                horizontalLineToRelative(0.76f)
                curveToRelative(0.55f, 0f, 1f, 0.45f, 1f, 1f)
                reflectiveCurveToRelative(-0.45f, 1f, -1f, 1f)
                close()
                moveTo(18f, 16f)
                horizontalLineToRelative(-0.77f)
                curveToRelative(0.07f, 0.69f, 0.15f, 1.32f, 0.23f, 1.86f)
                curveToRelative(0.08f, 0.55f, -0.3f, 1.05f, -0.85f, 1.13f)
                curveToRelative(-0.54f, 0.08f, -1.06f, -0.3f, -1.13f, -0.85f)
                curveToRelative(-0.09f, -0.61f, -0.18f, -1.34f, -0.26f, -2.14f)
                horizontalLineToRelative(-1.22f)
                curveToRelative(-0.55f, 0f, -1f, -0.45f, -1f, -1f)
                reflectiveCurveToRelative(0.45f, -1f, 1f, -1f)
                horizontalLineToRelative(1.06f)
                curveToRelative(-0.04f, -0.66f, -0.06f, -1.33f, -0.06f, -2f)
                curveToRelative(0f, -1.84f, 0.16f, -3.91f, 0.49f, -6.14f)
                curveToRelative(0.08f, -0.55f, 0.59f, -0.92f, 1.13f, -0.85f)
                curveToRelative(0.55f, 0.08f, 0.93f, 0.59f, 0.85f, 1.13f)
                curveToRelative(-0.3f, 2.11f, -0.47f, 4.13f, -0.47f, 5.86f)
                curveToRelative(0f, 0.67f, 0.03f, 1.35f, 0.07f, 2f)
                horizontalLineToRelative(0.94f)
                curveToRelative(0.55f, 0f, 1f, 0.45f, 1f, 1f)
                reflectiveCurveToRelative(-0.45f, 1f, -1f, 1f)
                close()
            }
        }.also { _SlidersVSquare = it}
