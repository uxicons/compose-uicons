package com.uxicons.compose.uicons.solid.chubby

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _UserInterfaceUi: ImageVector? = null

val Icons.Sc.UserInterfaceUi: ImageVector
    get() = _UserInterfaceUi ?: UXIcon(name = "UserInterfaceUi") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(21.98f, 2.79f)
                curveToRelative(-0.08f, -0.38f, -0.38f, -0.68f, -0.76f, -0.76f)
                curveToRelative(-0.19f, -0.04f, -4.71f, -1.02f, -9.21f, -1.02f)
                reflectiveCurveToRelative(-9.02f, 0.98f, -9.21f, 1.02f)
                curveToRelative(-0.38f, 0.08f, -0.68f, 0.38f, -0.76f, 0.76f)
                curveToRelative(-0.04f, 0.19f, -1.02f, 4.71f, -1.02f, 9.21f)
                reflectiveCurveToRelative(0.98f, 9.02f, 1.02f, 9.21f)
                curveToRelative(0.08f, 0.38f, 0.38f, 0.68f, 0.76f, 0.76f)
                curveToRelative(0.19f, 0.04f, 4.71f, 1.02f, 9.21f, 1.02f)
                reflectiveCurveToRelative(9.02f, -0.98f, 9.21f, -1.02f)
                curveToRelative(0.38f, -0.08f, 0.68f, -0.38f, 0.76f, -0.76f)
                curveToRelative(0.04f, -0.19f, 1.02f, -4.71f, 1.02f, -9.21f)
                reflectiveCurveToRelative(-0.98f, -9.02f, -1.02f, -9.21f)
                close()
                moveTo(10f, 17f)
                curveToRelative(-3.58f, 0f, -4f, -2.71f, -4f, -4.75f)
                curveToRelative(0f, -1.91f, 0.28f, -4.27f, 0.29f, -4.37f)
                curveToRelative(0.07f, -0.55f, 0.56f, -0.94f, 1.11f, -0.88f)
                curveToRelative(0.55f, 0.07f, 0.94f, 0.56f, 0.88f, 1.11f)
                curveToRelative(-0.0f, 0.02f, -0.27f, 2.34f, -0.27f, 4.13f)
                curveToRelative(0f, 2.56f, 0.79f, 2.75f, 2f, 2.75f)
                reflectiveCurveToRelative(2f, -0.19f, 2f, -2.75f)
                curveToRelative(0f, -1.79f, -0.27f, -4.11f, -0.27f, -4.13f)
                curveToRelative(-0.06f, -0.55f, 0.33f, -1.05f, 0.88f, -1.11f)
                curveToRelative(0.53f, -0.06f, 1.04f, 0.33f, 1.11f, 0.88f)
                curveToRelative(0.01f, 0.1f, 0.29f, 2.46f, 0.29f, 4.37f)
                curveToRelative(0f, 2.04f, -0.41f, 4.75f, -4f, 4.75f)
                close()
                moveTo(17.68f, 16.15f)
                curveToRelative(-0.08f, 0.55f, -0.6f, 0.92f, -1.14f, 0.84f)
                curveToRelative(-0.55f, -0.08f, -0.92f, -0.59f, -0.84f, -1.14f)
                curveToRelative(0.0f, -0.02f, 0.3f, -2.03f, 0.3f, -4.01f)
                curveToRelative(0f, -1.96f, -0.29f, -3.65f, -0.3f, -3.67f)
                curveToRelative(-0.1f, -0.54f, 0.27f, -1.06f, 0.81f, -1.16f)
                curveToRelative(0.54f, -0.1f, 1.06f, 0.27f, 1.16f, 0.81f)
                curveToRelative(0.01f, 0.08f, 0.33f, 1.88f, 0.33f, 4.02f)
                curveToRelative(0f, 2.13f, -0.31f, 4.22f, -0.32f, 4.3f)
                close()
            }
        }.also { _UserInterfaceUi = it}
