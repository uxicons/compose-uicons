package com.uxicons.compose.uicons.solid.chubby

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _ArrowDownFromDottedLine: ImageVector? = null

val Icons.Sc.ArrowDownFromDottedLine: ImageVector
    get() = _ArrowDownFromDottedLine ?: UXIcon(name = "ArrowDownFromDottedLine") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(18.75f, 16.82f)
                curveToRelative(-1.59f, 2.44f, -3.6f, 4.44f, -5.95f, 5.94f)
                curveToRelative(-0.49f, 0.32f, -1.12f, 0.32f, -1.62f, 0f)
                curveToRelative(-2.35f, -1.5f, -4.35f, -3.5f, -5.95f, -5.94f)
                curveToRelative(-0.45f, -0.69f, -0.26f, -1.62f, 0.43f, -2.08f)
                curveToRelative(0.69f, -0.45f, 1.62f, -0.26f, 2.08f, 0.43f)
                curveToRelative(0.8f, 1.22f, 1.72f, 2.3f, 2.75f, 3.25f)
                verticalLineToRelative(-10.93f)
                curveToRelative(0f, -0.83f, 0.67f, -1.5f, 1.5f, -1.5f)
                reflectiveCurveToRelative(1.5f, 0.67f, 1.5f, 1.5f)
                verticalLineToRelative(10.93f)
                curveToRelative(1.02f, -0.95f, 1.95f, -2.03f, 2.75f, -3.25f)
                curveToRelative(0.45f, -0.69f, 1.38f, -0.89f, 2.08f, -0.43f)
                curveToRelative(0.69f, 0.45f, 0.89f, 1.38f, 0.43f, 2.08f)
                close()
                moveTo(12f, 4f)
                curveToRelative(0.83f, 0f, 1.5f, -0.67f, 1.5f, -1.5f)
                reflectiveCurveToRelative(-0.67f, -1.5f, -1.5f, -1.5f)
                reflectiveCurveToRelative(-1.5f, 0.67f, -1.5f, 1.5f)
                reflectiveCurveToRelative(0.67f, 1.5f, 1.5f, 1.5f)
                close()
                moveTo(17f, 4f)
                curveToRelative(0.83f, 0f, 1.5f, -0.67f, 1.5f, -1.5f)
                reflectiveCurveToRelative(-0.67f, -1.5f, -1.5f, -1.5f)
                reflectiveCurveToRelative(-1.5f, 0.67f, -1.5f, 1.5f)
                reflectiveCurveToRelative(0.67f, 1.5f, 1.5f, 1.5f)
                close()
                moveTo(21.5f, 1f)
                curveToRelative(-0.83f, 0f, -1.5f, 0.67f, -1.5f, 1.5f)
                reflectiveCurveToRelative(0.67f, 1.5f, 1.5f, 1.5f)
                reflectiveCurveToRelative(1.5f, -0.67f, 1.5f, -1.5f)
                reflectiveCurveToRelative(-0.67f, -1.5f, -1.5f, -1.5f)
                close()
                moveTo(7f, 4f)
                curveToRelative(0.83f, 0f, 1.5f, -0.67f, 1.5f, -1.5f)
                reflectiveCurveToRelative(-0.67f, -1.5f, -1.5f, -1.5f)
                reflectiveCurveToRelative(-1.5f, 0.67f, -1.5f, 1.5f)
                reflectiveCurveToRelative(0.67f, 1.5f, 1.5f, 1.5f)
                close()
                moveTo(2.5f, 1f)
                curveToRelative(-0.83f, 0f, -1.5f, 0.67f, -1.5f, 1.5f)
                reflectiveCurveToRelative(0.67f, 1.5f, 1.5f, 1.5f)
                reflectiveCurveToRelative(1.5f, -0.67f, 1.5f, -1.5f)
                reflectiveCurveToRelative(-0.67f, -1.5f, -1.5f, -1.5f)
                close()
            }
        }.also { _ArrowDownFromDottedLine = it}
