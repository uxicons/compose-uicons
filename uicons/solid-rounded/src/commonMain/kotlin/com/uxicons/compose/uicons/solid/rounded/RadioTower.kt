package com.uxicons.compose.uicons.solid.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _RadioTower: ImageVector? = null

val Icons.Sr.RadioTower: ImageVector
    get() = _RadioTower ?: UXIcon(name = "RadioTower") {
            path(fill = SolidColor(Color.Black)) {
                moveTo(20.44f, 22.66f)
                curveToRelative(0.19f, 0.52f, -0.08f, 1.09f, -0.6f, 1.28f)
                curveToRelative(-0.11f, 0.04f, -0.23f, 0.06f, -0.34f, 0.06f)
                curveToRelative(-0.41f, 0f, -0.79f, -0.25f, -0.94f, -0.66f)
                lineTo(12f, 5.41f)
                lineToRelative(-3.51f, 9.59f)
                horizontalLineToRelative(4.51f)
                curveToRelative(0.55f, 0f, 1f, 0.45f, 1f, 1f)
                reflectiveCurveToRelative(-0.45f, 1f, -1f, 1f)
                lineTo(7.76f, 17f)
                lineToRelative(-1.1f, 3f)
                lineTo(15f, 20f)
                curveToRelative(0.55f, 0f, 1f, 0.45f, 1f, 1f)
                reflectiveCurveToRelative(-0.45f, 1f, -1f, 1f)
                lineTo(5.93f, 22f)
                lineToRelative(-0.49f, 1.34f)
                curveToRelative(-0.19f, 0.52f, -0.77f, 0.78f, -1.28f, 0.59f)
                curveToRelative(-0.52f, -0.19f, -0.79f, -0.76f, -0.6f, -1.28f)
                lineTo(10.6f, 3.42f)
                curveToRelative(-0.37f, -0.36f, -0.6f, -0.87f, -0.6f, -1.42f)
                curveToRelative(0f, -1.1f, 0.9f, -2f, 2f, -2f)
                reflectiveCurveToRelative(2f, 0.9f, 2f, 2f)
                curveToRelative(0f, 0.56f, -0.23f, 1.06f, -0.6f, 1.42f)
                lineToRelative(7.04f, 19.23f)
                close()
                moveTo(6.8f, 6f)
                curveToRelative(0.17f, 0f, 0.34f, -0.04f, 0.5f, -0.14f)
                curveToRelative(0.48f, -0.28f, 0.64f, -0.89f, 0.36f, -1.37f)
                curveToRelative(-0.44f, -0.76f, -0.67f, -1.62f, -0.67f, -2.5f)
                curveToRelative(0f, -0.28f, 0.02f, -0.56f, 0.07f, -0.83f)
                curveToRelative(0.09f, -0.55f, -0.28f, -1.06f, -0.82f, -1.15f)
                curveToRelative(-0.56f, -0.09f, -1.06f, 0.28f, -1.15f, 0.82f)
                curveToRelative(-0.06f, 0.38f, -0.1f, 0.77f, -0.1f, 1.17f)
                curveToRelative(0f, 1.23f, 0.32f, 2.44f, 0.94f, 3.5f)
                curveToRelative(0.19f, 0.32f, 0.52f, 0.5f, 0.87f, 0.5f)
                close()
                moveTo(16.7f, 5.87f)
                curveToRelative(0.47f, 0.27f, 1.09f, 0.12f, 1.37f, -0.36f)
                curveToRelative(0.61f, -1.06f, 0.94f, -2.27f, 0.94f, -3.5f)
                curveToRelative(0f, -0.39f, -0.03f, -0.78f, -0.1f, -1.17f)
                curveToRelative(-0.09f, -0.54f, -0.6f, -0.91f, -1.15f, -0.82f)
                curveToRelative(-0.55f, 0.09f, -0.91f, 0.61f, -0.82f, 1.15f)
                curveToRelative(0.05f, 0.27f, 0.07f, 0.55f, 0.07f, 0.83f)
                curveToRelative(0f, 0.88f, -0.23f, 1.74f, -0.67f, 2.5f)
                curveToRelative(-0.28f, 0.48f, -0.11f, 1.09f, 0.36f, 1.37f)
                close()
                moveTo(17.85f, 8.84f)
                curveToRelative(-0.42f, 0.36f, -0.47f, 0.99f, -0.11f, 1.41f)
                curveToRelative(0.36f, 0.42f, 0.99f, 0.47f, 1.41f, 0.11f)
                curveToRelative(2.45f, -2.09f, 3.85f, -5.14f, 3.85f, -8.36f)
                curveToRelative(0f, -0.37f, -0.02f, -0.74f, -0.06f, -1.1f)
                curveToRelative(-0.06f, -0.55f, -0.53f, -0.95f, -1.1f, -0.9f)
                curveToRelative(-0.55f, 0.06f, -0.95f, 0.55f, -0.9f, 1.1f)
                curveToRelative(0.03f, 0.3f, 0.04f, 0.6f, 0.04f, 0.9f)
                curveToRelative(0f, 2.63f, -1.15f, 5.13f, -3.15f, 6.84f)
                close()
                moveTo(4.85f, 10.36f)
                curveToRelative(0.42f, 0.36f, 1.05f, 0.31f, 1.41f, -0.11f)
                curveToRelative(0.36f, -0.42f, 0.31f, -1.05f, -0.11f, -1.41f)
                curveToRelative(-2.0f, -1.71f, -3.15f, -4.21f, -3.15f, -6.84f)
                curveToRelative(0f, -0.3f, 0.01f, -0.6f, 0.04f, -0.9f)
                curveTo(3.1f, 0.55f, 2.7f, 0.06f, 2.15f, 0.01f)
                curveToRelative(-0.55f, -0.05f, -1.04f, 0.35f, -1.1f, 0.9f)
                curveToRelative(-0.04f, 0.36f, -0.06f, 0.73f, -0.06f, 1.1f)
                curveToRelative(0f, 3.22f, 1.4f, 6.26f, 3.85f, 8.36f)
                close()
            }
        }.also { _RadioTower = it}
