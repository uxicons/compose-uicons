package com.uxicons.compose.uicons.solid.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _ReferralLinkArrow: ImageVector? = null

val Icons.Sr.ReferralLinkArrow: ImageVector
    get() = _ReferralLinkArrow ?: UXIcon(name = "ReferralLinkArrow") {
            path(fill = SolidColor(Color.Black)) {
                moveTo(23.42f, 17.59f)
                curveToRelative(0.77f, 0.78f, 0.77f, 2.04f, 0f, 2.81f)
                lineToRelative(-2.76f, 2.76f)
                curveToRelative(-0.2f, 0.2f, -0.45f, 0.29f, -0.71f, 0.29f)
                reflectiveCurveToRelative(-0.51f, -0.1f, -0.71f, -0.29f)
                curveToRelative(-0.39f, -0.39f, -0.39f, -1.02f, 0f, -1.41f)
                lineToRelative(1.75f, -1.75f)
                horizontalLineTo(1f)
                curveToRelative(-0.55f, 0f, -1f, -0.45f, -1f, -1f)
                reflectiveCurveToRelative(0.45f, -1f, 1f, -1f)
                horizontalLineTo(21.01f)
                lineToRelative(-1.76f, -1.75f)
                curveToRelative(-0.39f, -0.39f, -0.39f, -1.02f, 0f, -1.41f)
                reflectiveCurveToRelative(1.02f, -0.39f, 1.41f, 0f)
                lineToRelative(2.76f, 2.76f)
                close()
                moveTo(17.91f, 2.02f)
                curveToRelative(2.33f, 0.21f, 4.09f, 2.14f, 4.09f, 4.48f)
                curveToRelative(0f, 2.48f, -2.02f, 4.5f, -4.5f, 4.5f)
                horizontalLineToRelative(-4f)
                curveToRelative(-2.48f, 0f, -4.5f, -2.02f, -4.5f, -4.5f)
                curveToRelative(0f, -0.42f, 0.06f, -0.83f, 0.17f, -1.23f)
                curveToRelative(0.15f, -0.53f, -0.16f, -1.08f, -0.69f, -1.23f)
                curveToRelative(-0.53f, -0.15f, -1.08f, 0.16f, -1.23f, 0.69f)
                curveToRelative(-0.16f, 0.57f, -0.24f, 1.17f, -0.24f, 1.77f)
                curveToRelative(0f, 3.58f, 2.92f, 6.5f, 6.5f, 6.5f)
                horizontalLineToRelative(4f)
                curveToRelative(3.58f, 0f, 6.5f, -2.92f, 6.5f, -6.5f)
                curveTo(24f, 3.11f, 21.46f, 0.33f, 18.09f, 0.03f)
                curveToRelative(-0.54f, -0.04f, -1.04f, 0.36f, -1.09f, 0.91f)
                reflectiveCurveToRelative(0.36f, 1.04f, 0.91f, 1.09f)
                close()
                moveTo(5.91f, 13.0f)
                curveToRelative(0.03f, 0.0f, 0.06f, 0.0f, 0.09f, 0.0f)
                curveToRelative(0.51f, 0f, 0.95f, -0.39f, 0.99f, -0.91f)
                curveToRelative(0.05f, -0.55f, -0.36f, -1.04f, -0.91f, -1.09f)
                curveToRelative(-2.33f, -0.21f, -4.09f, -2.14f, -4.09f, -4.48f)
                curveToRelative(0f, -2.48f, 2.02f, -4.5f, 4.5f, -4.5f)
                horizontalLineToRelative(4f)
                curveToRelative(2.48f, 0f, 4.5f, 2.02f, 4.5f, 4.5f)
                curveToRelative(0f, 0.42f, -0.06f, 0.83f, -0.17f, 1.23f)
                curveToRelative(-0.15f, 0.53f, 0.16f, 1.08f, 0.69f, 1.23f)
                curveToRelative(0.53f, 0.15f, 1.08f, -0.16f, 1.23f, -0.69f)
                curveToRelative(0.16f, -0.57f, 0.24f, -1.17f, 0.24f, -1.77f)
                curveTo(17f, 2.94f, 14.08f, 0.02f, 10.5f, 0.02f)
                horizontalLineTo(6.5f)
                curveTo(2.92f, 0.02f, 0f, 2.94f, 0f, 6.52f)
                curveToRelative(0f, 3.39f, 2.54f, 6.17f, 5.91f, 6.47f)
                close()
            }
        }.also { _ReferralLinkArrow = it}
