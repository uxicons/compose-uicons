package com.uxicons.compose.uicons.solid.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _HighFiveCelebrationYes: ImageVector? = null

val Icons.Sr.HighFiveCelebrationYes: ImageVector
    get() = _HighFiveCelebrationYes ?: UXIcon(name = "HighFiveCelebrationYes") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(15f, 17.57f)
                horizontalLineToRelative(0f)
                verticalLineToRelative(1.43f)
                curveToRelative(0f, 2.76f, -2.24f, 5f, -5f, 5f)
                horizontalLineToRelative(-2.6f)
                curveToRelative(-1.53f, 0f, -2.99f, -0.61f, -4.07f, -1.69f)
                lineToRelative(-2.74f, -2.62f)
                curveToRelative(-0.86f, -0.86f, -0.77f, -2.31f, 0.26f, -3.05f)
                curveToRelative(0.81f, -0.58f, 1.94f, -0.41f, 2.65f, 0.29f)
                lineToRelative(0.51f, 0.53f)
                verticalLineToRelative(-7.93f)
                curveToRelative(0f, -0.81f, 0.62f, -1.5f, 1.43f, -1.54f)
                curveToRelative(0.86f, -0.04f, 1.57f, 0.65f, 1.57f, 1.5f)
                verticalLineToRelative(5.0f)
                curveToRelative(0f, 0.28f, 0.22f, 0.5f, 0.5f, 0.5f)
                horizontalLineToRelative(0.01f)
                curveToRelative(0.28f, 0f, 0.5f, -0.22f, 0.5f, -0.5f)
                curveToRelative(-0.0f, -2.31f, -0.01f, -5.45f, -0.01f, -6.0f)
                curveToRelative(0f, -0.83f, 0.67f, -1.5f, 1.5f, -1.5f)
                reflectiveCurveToRelative(1.5f, 0.68f, 1.5f, 1.5f)
                lineToRelative(0.0f, 6.0f)
                curveToRelative(0f, 0.28f, 0.22f, 0.5f, 0.5f, 0.5f)
                horizontalLineToRelative(0f)
                curveToRelative(0.28f, 0f, 0.5f, -0.22f, 0.5f, -0.5f)
                verticalLineToRelative(-3.96f)
                curveToRelative(0f, -0.81f, 0.62f, -1.5f, 1.43f, -1.54f)
                curveToRelative(0.86f, -0.04f, 1.57f, 0.65f, 1.57f, 1.5f)
                horizontalLineToRelative(0f)
                reflectiveCurveToRelative(0f, 0.01f, 0f, 0.01f)
                verticalLineToRelative(7.07f)
                close()
                moveTo(23.16f, 9.65f)
                curveToRelative(-0.81f, -0.58f, -1.94f, -0.41f, -2.65f, 0.29f)
                lineToRelative(-0.51f, 0.53f)
                lineTo(20f, 2.54f)
                curveToRelative(0f, -0.81f, -0.62f, -1.5f, -1.43f, -1.54f)
                curveToRelative(-0.86f, -0.04f, -1.57f, 0.65f, -1.57f, 1.5f)
                verticalLineToRelative(5.0f)
                curveToRelative(0f, 0.28f, -0.23f, 0.51f, -0.51f, 0.51f)
                horizontalLineToRelative(0f)
                curveToRelative(-0.28f, 0f, -0.51f, -0.23f, -0.51f, -0.51f)
                curveToRelative(0.0f, -2.31f, 0.01f, -5.45f, 0.01f, -6.0f)
                curveToRelative(0f, -0.83f, -0.67f, -1.5f, -1.5f, -1.5f)
                reflectiveCurveToRelative(-1.5f, 0.68f, -1.5f, 1.5f)
                lineToRelative(-0.0f, 5.54f)
                curveToRelative(1.05f, -0.15f, 2.2f, 0.2f, 2.98f, 0.98f)
                curveToRelative(0.64f, 0.64f, 1.0f, 1.49f, 1.02f, 2.39f)
                verticalLineToRelative(6.56f)
                curveToRelative(1.38f, -0.1f, 2.69f, -0.68f, 3.67f, -1.67f)
                lineToRelative(2.74f, -2.62f)
                curveToRelative(0.86f, -0.86f, 0.77f, -2.31f, -0.26f, -3.05f)
                close()
                moveTo(9.5f, 5.0f)
                curveToRelative(0.94f, 0f, 1.82f, 0.37f, 2.48f, 1.03f)
                curveToRelative(0.01f, 0.01f, 0.01f, 0.01f, 0.02f, 0.02f)
                verticalLineToRelative(-2.55f)
                curveToRelative(0f, -0.83f, -0.67f, -1.5f, -1.5f, -1.5f)
                reflectiveCurveToRelative(-1.5f, 0.67f, -1.5f, 1.5f)
                horizontalLineToRelative(0f)
                reflectiveCurveToRelative(0f, 1.55f, 0f, 1.55f)
                curveToRelative(0.17f, -0.02f, 0.33f, -0.06f, 0.5f, -0.06f)
                close()
            }
        }.also { _HighFiveCelebrationYes = it}
