package com.uxicons.compose.uicons.thin.straight

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _VoiceNote: ImageVector? = null

val Icons.Ts.VoiceNote: ImageVector
    get() = _VoiceNote ?: UXIcon(name = "VoiceNote") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(12f, 8f)
                curveToRelative(-1.1f, 0f, -2f, 0.9f, -2f, 2f)
                reflectiveCurveToRelative(0.9f, 2f, 2f, 2f)
                reflectiveCurveToRelative(2f, -0.9f, 2f, -2f)
                reflectiveCurveToRelative(-0.9f, -2f, -2f, -2f)
                close()
                moveTo(12f, 11f)
                curveToRelative(-0.55f, 0f, -1f, -0.45f, -1f, -1f)
                reflectiveCurveToRelative(0.45f, -1f, 1f, -1f)
                reflectiveCurveToRelative(1f, 0.45f, 1f, 1f)
                reflectiveCurveToRelative(-0.45f, 1f, -1f, 1f)
                close()
                moveTo(7.05f, 14.95f)
                lineTo(6.34f, 15.66f)
                curveToRelative(-3.12f, -3.12f, -3.12f, -8.2f, 0f, -11.31f)
                lineToRelative(0.71f, 0.71f)
                curveToRelative(-2.73f, 2.73f, -2.73f, 7.17f, 0f, 9.9f)
                close()
                moveTo(17.66f, 4.34f)
                curveToRelative(3.12f, 3.12f, 3.12f, 8.2f, 0f, 11.31f)
                lineToRelative(-0.71f, -0.71f)
                curveToRelative(2.73f, -2.73f, 2.73f, -7.17f, 0f, -9.9f)
                close()
                moveTo(9.17f, 7.17f)
                curveToRelative(-0.76f, 0.76f, -1.17f, 1.76f, -1.17f, 2.83f)
                reflectiveCurveToRelative(0.42f, 2.07f, 1.17f, 2.83f)
                lineToRelative(-0.71f, 0.71f)
                curveToRelative(-0.94f, -0.94f, -1.46f, -2.2f, -1.46f, -3.54f)
                reflectiveCurveToRelative(0.52f, -2.59f, 1.46f, -3.54f)
                close()
                moveTo(17.0f, 10f)
                curveToRelative(0f, 1.34f, -0.52f, 2.59f, -1.46f, 3.54f)
                lineToRelative(-0.71f, -0.71f)
                curveToRelative(0.76f, -0.76f, 1.17f, -1.76f, 1.17f, -2.83f)
                reflectiveCurveToRelative(-0.42f, -2.07f, -1.17f, -2.83f)
                lineToRelative(0.71f, -0.71f)
                curveToRelative(0.94f, 0.94f, 1.46f, 2.2f, 1.46f, 3.54f)
                close()
                moveTo(21.5f, 0f)
                horizontalLineToRelative(-19f)
                curveToRelative(-1.38f, 0f, -2.5f, 1.12f, -2.5f, 2.5f)
                verticalLineToRelative(17.5f)
                horizontalLineToRelative(6.78f)
                lineToRelative(4.22f, 3.62f)
                curveToRelative(0.28f, 0.24f, 0.63f, 0.38f, 1.0f, 0.38f)
                reflectiveCurveToRelative(0.72f, -0.14f, 0.99f, -0.37f)
                lineToRelative(4.22f, -3.63f)
                horizontalLineToRelative(6.79f)
                verticalLineToRelative(-17.5f)
                curveToRelative(0f, -1.38f, -1.12f, -2.5f, -2.5f, -2.5f)
                close()
                moveTo(23f, 19f)
                horizontalLineToRelative(-6.16f)
                lineToRelative(-4.5f, 3.87f)
                curveToRelative(-0.18f, 0.16f, -0.47f, 0.17f, -0.67f, -0.01f)
                lineToRelative(-4.51f, -3.87f)
                horizontalLineToRelative(-6.15f)
                verticalLineToRelative(-16.5f)
                curveToRelative(0f, -0.83f, 0.67f, -1.5f, 1.5f, -1.5f)
                horizontalLineToRelative(19f)
                curveToRelative(0.83f, 0f, 1.5f, 0.67f, 1.5f, 1.5f)
                close()
            }
        }.also { _VoiceNote = it}
