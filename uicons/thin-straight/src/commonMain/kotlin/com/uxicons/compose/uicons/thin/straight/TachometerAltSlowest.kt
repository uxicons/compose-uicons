package com.uxicons.compose.uicons.thin.straight

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _TachometerAltSlowest: ImageVector? = null

val Icons.Ts.TachometerAltSlowest: ImageVector
    get() = _TachometerAltSlowest ?: UXIcon(name = "TachometerAltSlowest") {
            path(fill = SolidColor(Color.Black)) {
                moveTo(12f, 1f)
                curveTo(5.38f, 1f, 0f, 6.38f, 0f, 13f)
                curveToRelative(0f, 3.91f, 1.95f, 7.62f, 5.21f, 9.91f)
                lineToRelative(0.13f, 0.09f)
                horizontalLineToRelative(13.32f)
                lineToRelative(0.13f, -0.09f)
                curveToRelative(3.26f, -2.29f, 5.21f, -6.0f, 5.21f, -9.91f)
                curveToRelative(0f, -6.62f, -5.38f, -12f, -12f, -12f)
                close()
                moveTo(18.34f, 22f)
                lineTo(5.66f, 22f)
                curveToRelative(-2.88f, -2.08f, -4.66f, -5.52f, -4.66f, -9f)
                curveTo(1f, 6.93f, 5.93f, 2f, 12f, 2f)
                reflectiveCurveToRelative(11f, 4.93f, 11f, 11f)
                curveToRelative(0f, 3.48f, -1.78f, 6.92f, -4.66f, 9f)
                close()
                moveTo(12f, 11f)
                curveToRelative(-1.1f, 0f, -2f, 0.9f, -2f, 2f)
                curveToRelative(0f, 0.12f, 0.01f, 0.25f, 0.03f, 0.37f)
                lineToRelative(-5.75f, 2.68f)
                lineToRelative(0.42f, 0.91f)
                lineToRelative(5.75f, -2.68f)
                curveToRelative(0.37f, 0.44f, 0.92f, 0.73f, 1.54f, 0.73f)
                curveToRelative(1.1f, 0f, 2f, -0.9f, 2f, -2f)
                reflectiveCurveToRelative(-0.9f, -2f, -2f, -2f)
                close()
                moveTo(12f, 14f)
                curveToRelative(-0.55f, 0f, -1f, -0.45f, -1f, -1f)
                reflectiveCurveToRelative(0.45f, -1f, 1f, -1f)
                reflectiveCurveToRelative(1f, 0.45f, 1f, 1f)
                reflectiveCurveToRelative(-0.45f, 1f, -1f, 1f)
                close()
                moveTo(17.66f, 18.66f)
                lineToRelative(-0.71f, -0.71f)
                curveToRelative(2.73f, -2.73f, 2.73f, -7.17f, 0f, -9.9f)
                curveToRelative(-2.73f, -2.73f, -7.17f, -2.73f, -9.9f, 0f)
                curveToRelative(-1.32f, 1.32f, -2.05f, 3.08f, -2.05f, 4.95f)
                curveToRelative(0f, 0.17f, 0.01f, 0.33f, 0.03f, 0.5f)
                lineToRelative(-0.96f, 0.45f)
                curveToRelative(-0.04f, -0.31f, -0.06f, -0.63f, -0.06f, -0.95f)
                curveToRelative(0f, -2.14f, 0.83f, -4.15f, 2.34f, -5.66f)
                curveToRelative(3.12f, -3.12f, 8.19f, -3.12f, 11.31f, 0f)
                reflectiveCurveToRelative(3.12f, 8.2f, 0f, 11.31f)
                close()
            }
        }.also { _TachometerAltSlowest = it}
