package com.uxicons.compose.uicons.regular.chubby

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _InboxOut: ImageVector? = null

val Icons.Rc.InboxOut: ImageVector
    get() = _InboxOut ?: UXIcon(name = "InboxOut") {
            path(fill = SolidColor(Color.Black)) {
                moveTo(22f, 10.62f)
                horizontalLineToRelative(-4.78f)
                curveToRelative(-0.52f, 0f, -0.95f, 0.4f, -1.0f, 0.91f)
                curveToRelative(-0.09f, 0.99f, -0.25f, 1.81f, -0.36f, 2.31f)
                curveToRelative(-0.77f, 0.17f, -2.24f, 0.44f, -3.86f, 0.44f)
                reflectiveCurveToRelative(-3.1f, -0.27f, -3.86f, -0.44f)
                curveToRelative(-0.11f, -0.5f, -0.27f, -1.31f, -0.36f, -2.31f)
                curveToRelative(-0.05f, -0.52f, -0.48f, -0.91f, -1.0f, -0.91f)
                lineTo(2f, 10.62f)
                curveToRelative(-0.55f, 0f, -1f, 0.45f, -1f, 1f)
                curveToRelative(0f, 5.17f, 1.15f, 8.98f, 1.19f, 9.14f)
                curveToRelative(0.1f, 0.33f, 0.36f, 0.58f, 0.69f, 0.67f)
                curveToRelative(0.17f, 0.05f, 4.27f, 1.19f, 9.12f, 1.19f)
                reflectiveCurveToRelative(8.95f, -1.14f, 9.12f, -1.19f)
                curveToRelative(0.33f, -0.09f, 0.59f, -0.35f, 0.69f, -0.67f)
                curveToRelative(0.05f, -0.16f, 1.19f, -4.01f, 1.19f, -9.14f)
                curveToRelative(0f, -0.55f, -0.45f, -1f, -1f, -1f)
                close()
                moveTo(20.04f, 19.64f)
                curveToRelative(-1.25f, 0.3f, -4.47f, 0.98f, -8.04f, 0.98f)
                reflectiveCurveToRelative(-6.8f, -0.68f, -8.04f, -0.98f)
                curveToRelative(-0.27f, -1.08f, -0.85f, -3.7f, -0.95f, -7.02f)
                horizontalLineToRelative(2.87f)
                curveToRelative(0.19f, 1.4f, 0.46f, 2.29f, 0.47f, 2.34f)
                curveToRelative(0.1f, 0.33f, 0.36f, 0.58f, 0.69f, 0.67f)
                curveToRelative(0.09f, 0.03f, 2.32f, 0.65f, 4.95f, 0.65f)
                reflectiveCurveToRelative(4.86f, -0.62f, 4.95f, -0.65f)
                curveToRelative(0.33f, -0.09f, 0.59f, -0.35f, 0.69f, -0.67f)
                curveToRelative(0.01f, -0.04f, 0.28f, -0.95f, 0.47f, -2.34f)
                horizontalLineToRelative(2.87f)
                curveToRelative(-0.1f, 3.29f, -0.68f, 5.94f, -0.95f, 7.02f)
                close()
            }
            path(fill = SolidColor(Color.Black)) {
                moveTo(8.18f, 6.79f)
                curveToRelative(0.49f, 0.25f, 1.09f, 0.06f, 1.35f, -0.43f)
                curveToRelative(0.48f, -0.93f, 1.0f, -1.59f, 1.47f, -2.05f)
                verticalLineToRelative(6.11f)
                curveToRelative(0f, 0.55f, 0.45f, 1f, 1f, 1f)
                reflectiveCurveToRelative(1f, -0.45f, 1f, -1f)
                verticalLineTo(4.3f)
                curveToRelative(0.47f, 0.47f, 0.99f, 1.13f, 1.47f, 2.05f)
                curveToRelative(0.18f, 0.34f, 0.53f, 0.54f, 0.89f, 0.54f)
                curveToRelative(0.15f, 0f, 0.31f, -0.04f, 0.46f, -0.11f)
                curveToRelative(0.49f, -0.25f, 0.68f, -0.86f, 0.43f, -1.35f)
                curveToRelative(-1.61f, -3.11f, -3.81f, -3.96f, -3.9f, -3.99f)
                curveToRelative(-0.23f, -0.09f, -0.47f, -0.09f, -0.7f, 0f)
                curveToRelative(-0.09f, 0.04f, -2.29f, 0.89f, -3.9f, 3.99f)
                curveToRelative(-0.25f, 0.49f, -0.06f, 1.09f, 0.43f, 1.35f)
                close()
            }
        }.also { _InboxOut = it}
