package com.uxicons.compose.uicons.bold.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _ShieldDog: ImageVector? = null

val Icons.Br.ShieldDog: ImageVector
    get() = _ShieldDog ?: UXIcon(name = "ShieldDog") {
        path(fill = SolidColor(Color.Black)) {
            moveToRelative(18.57f, 2.13f)
            lineTo(12.31f, 0.05f)
            curveToRelative(-0.2f, -0.07f, -0.42f, -0.07f, -0.63f, 0f)
            lineToRelative(-6.26f, 2.08f)
            curveToRelative(-2.05f, 0.68f, -3.43f, 2.59f, -3.43f, 4.75f)
            verticalLineToRelative(5.17f)
            curveToRelative(0f, 6.56f, 7.0f, 10.58f, 9.15f, 11.65f)
            curveToRelative(0f, 0f, 0.48f, 0.31f, 0.85f, 0.31f)
            curveToRelative(0.4f, 0f, 0.96f, -0.31f, 1.06f, -0.35f)
            curveToRelative(2.42f, -1.03f, 8.94f, -4.46f, 8.94f, -11.61f)
            verticalLineToRelative(-5.17f)
            curveToRelative(0f, -2.16f, -1.38f, -4.07f, -3.43f, -4.75f)
            close()
            moveTo(20.0f, 12.04f)
            curveToRelative(0f, 6.18f, -6.09f, 9.11f, -7.95f, 9.86f)
            curveToRelative(-1.89f, -0.94f, -8.05f, -4.45f, -8.05f, -9.86f)
            verticalLineToRelative(-5.17f)
            curveToRelative(0f, -1.3f, 0.83f, -2.44f, 2.06f, -2.85f)
            lineToRelative(5.94f, -1.97f)
            lineToRelative(5.95f, 1.97f)
            curveToRelative(1.23f, 0.41f, 2.06f, 1.55f, 2.06f, 2.85f)
            verticalLineToRelative(5.17f)
            close()
            moveTo(16.0f, 9.5f)
            curveToRelative(-0.39f, 0f, -0.95f, -0.25f, -1.27f, -0.74f)
            curveToRelative(-0.73f, -1.1f, -2.04f, -1.76f, -3.49f, -1.76f)
            horizontalLineToRelative(-0.24f)
            verticalLineToRelative(-1.79f)
            curveToRelative(0f, -0.5f, -0.52f, -0.85f, -0.98f, -0.65f)
            curveToRelative(-0.89f, 0.38f, -1.52f, 1.27f, -1.52f, 2.3f)
            verticalLineToRelative(0.9f)
            reflectiveCurveToRelative(-1.58f, 2.98f, -1.58f, 2.98f)
            curveToRelative(-0.47f, 0.93f, -0.55f, 2.0f, -0.23f, 2.93f)
            curveToRelative(0.58f, 1.65f, 1.98f, 3.07f, 3.06f, 3.96f)
            curveToRelative(0.27f, 0.23f, 0.61f, 0.35f, 0.96f, 0.35f)
            curveToRelative(0.15f, 0f, 0.3f, -0.02f, 0.45f, -0.07f)
            curveToRelative(0.48f, -0.15f, 0.84f, -0.53f, 0.98f, -1.01f)
            lineToRelative(0.52f, -1.89f)
            horizontalLineToRelative(1.64f)
            curveToRelative(1.68f, 0f, 3.16f, -1.11f, 3.58f, -2.7f)
            curveToRelative(0.27f, -1.02f, -0.0f, -1.66f, -0.28f, -2.03f)
            curveToRelative(-0.56f, -0.73f, -1.44f, -0.77f, -1.61f, -0.77f)
            close()
            moveTo(15.96f, 11.78f)
            curveToRelative(-0.19f, 0.72f, -0.87f, 1.22f, -1.65f, 1.22f)
            horizontalLineToRelative(-2.4f)
            curveToRelative(-0.45f, 0f, -0.84f, 0.3f, -0.96f, 0.73f)
            lineToRelative(-0.5f, 1.82f)
            curveToRelative(-0.93f, -0.88f, -1.57f, -1.76f, -1.85f, -2.55f)
            curveToRelative(-0.15f, -0.42f, -0.1f, -0.92f, 0.12f, -1.35f)
            lineToRelative(1.4f, -2.65f)
            horizontalLineToRelative(1.14f)
            curveToRelative(0.77f, 0f, 1.47f, 0.33f, 1.82f, 0.87f)
            curveToRelative(0.66f, 0.99f, 1.81f, 1.63f, 2.94f, 1.63f)
            verticalLineToRelative(-0.05f)
            reflectiveCurveToRelative(0.02f, 0.09f, -0.04f, 0.33f)
            close()
        }
    }.also { _ShieldDog = it }
