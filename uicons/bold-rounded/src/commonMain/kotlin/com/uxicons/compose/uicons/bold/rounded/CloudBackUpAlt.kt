package com.uxicons.compose.uicons.bold.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _CloudBackUpAlt: ImageVector? = null

val Icons.Br.CloudBackUpAlt: ImageVector
    get() = _CloudBackUpAlt ?: UXIcon(name = "CloudBackUpAlt") {
        path(fill = SolidColor(Color.Black)) {
            moveToRelative(18f, 18f)
            curveToRelative(0f, 3.31f, -2.69f, 6f, -6f, 6f)
            reflectiveCurveToRelative(-6f, -2.69f, -6f, -6f)
            curveToRelative(0f, -0.22f, 0.01f, -0.44f, 0.04f, -0.66f)
            curveToRelative(0.09f, -0.82f, 0.82f, -1.43f, 1.65f, -1.33f)
            curveToRelative(0.82f, 0.09f, 1.42f, 0.83f, 1.33f, 1.65f)
            curveToRelative(-0.01f, 0.11f, -0.02f, 0.23f, -0.02f, 0.34f)
            curveToRelative(0f, 1.65f, 1.35f, 3f, 3f, 3f)
            reflectiveCurveToRelative(3f, -1.35f, 3f, -3f)
            curveToRelative(0f, -1.3f, -0.84f, -2.4f, -2f, -2.82f)
            verticalLineToRelative(1.27f)
            curveToRelative(0f, 0.45f, -0.53f, 0.69f, -0.87f, 0.39f)
            lineToRelative(-2.71f, -2.39f)
            curveToRelative(-0.56f, -0.48f, -0.56f, -1.32f, 0f, -1.8f)
            lineToRelative(2.71f, -2.39f)
            curveToRelative(0.34f, -0.3f, 0.87f, -0.06f, 0.87f, 0.39f)
            verticalLineToRelative(1.43f)
            curveToRelative(2.83f, 0.48f, 5f, 2.94f, 5f, 5.91f)
            close()
            moveTo(17.81f, 4.95f)
            curveToRelative(-0.15f, -0.03f, -0.26f, -0.12f, -0.32f, -0.24f)
            curveTo(15.9f, 1.36f, 12.23f, -0.51f, 8.56f, 0.16f)
            curveTo(5.21f, 0.78f, 2.58f, 3.45f, 2.02f, 6.82f)
            curveToRelative(-0.16f, 0.99f, -0.15f, 1.98f, 0.03f, 2.95f)
            curveToRelative(0.02f, 0.1f, -0.05f, 0.25f, -0.16f, 0.35f)
            curveToRelative(-1.19f, 1.08f, -1.9f, 2.73f, -1.9f, 4.4f)
            curveToRelative(0f, 1.34f, 0.49f, 2.63f, 1.38f, 3.64f)
            curveToRelative(0.3f, 0.34f, 0.71f, 0.51f, 1.13f, 0.51f)
            curveToRelative(0.35f, 0f, 0.71f, -0.12f, 0.99f, -0.38f)
            curveToRelative(0.62f, -0.55f, 0.68f, -1.5f, 0.13f, -2.12f)
            curveToRelative(-0.4f, -0.46f, -0.62f, -1.04f, -0.62f, -1.65f)
            curveToRelative(0f, -0.82f, 0.36f, -1.68f, 0.92f, -2.18f)
            curveToRelative(0.89f, -0.81f, 1.31f, -2.01f, 1.09f, -3.14f)
            curveToRelative(-0.12f, -0.62f, -0.13f, -1.25f, -0.02f, -1.89f)
            curveToRelative(0.35f, -2.09f, 2.04f, -3.81f, 4.12f, -4.19f)
            curveToRelative(2.37f, -0.43f, 4.66f, 0.72f, 5.68f, 2.87f)
            curveToRelative(0.47f, 0.99f, 1.36f, 1.68f, 2.45f, 1.9f)
            curveToRelative(2.19f, 0.44f, 3.78f, 2.38f, 3.78f, 4.64f)
            curveToRelative(0f, 0.89f, -0.26f, 1.75f, -0.75f, 2.49f)
            curveToRelative(-0.46f, 0.69f, -0.27f, 1.62f, 0.42f, 2.08f)
            reflectiveCurveToRelative(1.62f, 0.27f, 2.08f, -0.42f)
            curveToRelative(0.82f, -1.23f, 1.25f, -2.67f, 1.25f, -4.17f)
            curveToRelative(0f, -3.66f, -2.6f, -6.84f, -6.19f, -7.55f)
            close()
        }
    }.also { _CloudBackUpAlt = it }
