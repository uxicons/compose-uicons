package com.uxicons.compose.uicons.bold.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _FolderCamera: ImageVector? = null

val Icons.Br.FolderCamera: ImageVector
    get() = _FolderCamera ?: UXIcon(name = "FolderCamera") {
        path(fill = SolidColor(Color.Black)) {
            moveTo(21.8f, 15f)
            lineToRelative(-0.84f, -1.31f)
            curveToRelative(-0.28f, -0.43f, -0.75f, -0.69f, -1.26f, -0.69f)
            horizontalLineToRelative(-2.4f)
            curveToRelative(-0.51f, 0f, -0.99f, 0.26f, -1.26f, 0.69f)
            lineToRelative(-0.84f, 1.31f)
            horizontalLineToRelative(0f)
            curveToRelative(-1.21f, 0f, -2.2f, 0.98f, -2.2f, 2.2f)
            verticalLineToRelative(4.61f)
            curveToRelative(0f, 1.21f, 0.98f, 2.2f, 2.2f, 2.2f)
            horizontalLineToRelative(6.61f)
            curveToRelative(1.21f, 0f, 2.2f, -0.98f, 2.2f, -2.2f)
            verticalLineToRelative(-4.61f)
            curveToRelative(0f, -1.21f, -0.98f, -2.2f, -2.2f, -2.2f)
            close()
            moveTo(18.5f, 21.5f)
            curveToRelative(-1.1f, 0f, -2f, -0.9f, -2f, -2f)
            reflectiveCurveToRelative(0.9f, -2f, 2f, -2f)
            reflectiveCurveToRelative(2f, 0.9f, 2f, 2f)
            reflectiveCurveToRelative(-0.9f, 2f, -2f, 2f)
            close()
            moveTo(24f, 7.5f)
            verticalLineToRelative(3f)
            curveToRelative(0f, 0.83f, -0.67f, 1.5f, -1.5f, 1.5f)
            reflectiveCurveToRelative(-1.5f, -0.67f, -1.5f, -1.5f)
            verticalLineToRelative(-3f)
            curveToRelative(0f, -0.17f, -0.02f, -0.34f, -0.05f, -0.5f)
            lineTo(3f, 7f)
            verticalLineToRelative(9.5f)
            curveToRelative(0f, 1.38f, 1.12f, 2.5f, 2.5f, 2.5f)
            horizontalLineToRelative(4f)
            curveToRelative(0.83f, 0f, 1.5f, 0.67f, 1.5f, 1.5f)
            reflectiveCurveToRelative(-0.67f, 1.5f, -1.5f, 1.5f)
            lineTo(5.5f, 22f)
            curveToRelative(-3.03f, 0f, -5.5f, -2.47f, -5.5f, -5.5f)
            lineTo(0f, 5.5f)
            curveTo(0f, 2.47f, 2.47f, 0f, 5.5f, 0f)
            horizontalLineToRelative(2.53f)
            curveToRelative(0.54f, 0f, 1.08f, 0.13f, 1.56f, 0.37f)
            lineToRelative(3.16f, 1.58f)
            curveToRelative(0.07f, 0.03f, 0.15f, 0.05f, 0.22f, 0.05f)
            horizontalLineToRelative(5.53f)
            curveToRelative(3.03f, 0f, 5.5f, 2.47f, 5.5f, 5.5f)
            close()
        }
    }.also { _FolderCamera = it }
