package com.uxicons.compose.uicons.regular.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _FluxCapacitor: ImageVector? = null

val Icons.Rr.FluxCapacitor: ImageVector
    get() = _FluxCapacitor ?: UXIcon(name = "FluxCapacitor") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(19f, 0f)
                lineTo(5f, 0f)
                curveTo(2.24f, 0f, 0f, 2.24f, 0f, 5f)
                verticalLineToRelative(14f)
                curveToRelative(0f, 2.76f, 2.24f, 5f, 5f, 5f)
                horizontalLineToRelative(14f)
                curveToRelative(2.76f, 0f, 5f, -2.24f, 5f, -5f)
                lineTo(24f, 5f)
                curveToRelative(0f, -2.76f, -2.24f, -5f, -5f, -5f)
                close()
                moveTo(22f, 19f)
                curveToRelative(0f, 1.65f, -1.35f, 3f, -3f, 3f)
                lineTo(5f, 22f)
                curveToRelative(-1.65f, 0f, -3f, -1.35f, -3f, -3f)
                lineTo(2f, 5f)
                curveToRelative(0f, -1.65f, 1.35f, -3f, 3f, -3f)
                horizontalLineToRelative(14f)
                curveToRelative(1.65f, 0f, 3f, 1.35f, 3f, 3f)
                verticalLineToRelative(14f)
                close()
                moveTo(11f, 16f)
                verticalLineToRelative(-4.0f)
                lineToRelative(-3.8f, -2.9f)
                curveToRelative(-0.44f, -0.33f, -0.52f, -0.96f, -0.19f, -1.4f)
                curveToRelative(0.34f, -0.44f, 0.96f, -0.53f, 1.4f, -0.19f)
                lineToRelative(3.59f, 2.73f)
                lineToRelative(3.59f, -2.73f)
                curveToRelative(0.44f, -0.34f, 1.07f, -0.25f, 1.4f, 0.19f)
                curveToRelative(0.33f, 0.44f, 0.25f, 1.07f, -0.19f, 1.4f)
                lineToRelative(-3.8f, 2.9f)
                verticalLineToRelative(4.0f)
                curveToRelative(0f, 0.55f, -0.45f, 1f, -1f, 1f)
                reflectiveCurveToRelative(-1f, -0.45f, -1f, -1f)
                close()
                moveTo(7.44f, 11.16f)
                curveToRelative(-0.15f, 0.41f, -0.53f, 0.67f, -0.94f, 0.67f)
                curveToRelative(-0.11f, 0f, -0.22f, -0.02f, -0.33f, -0.06f)
                curveToRelative(-1.6f, -0.56f, -2.67f, -2.08f, -2.67f, -3.77f)
                curveToRelative(0f, -2.21f, 1.79f, -4f, 4f, -4f)
                curveToRelative(1.42f, 0f, 2.75f, 0.77f, 3.46f, 2f)
                curveToRelative(0.28f, 0.48f, 0.11f, 1.09f, -0.36f, 1.37f)
                curveToRelative(-0.48f, 0.28f, -1.09f, 0.11f, -1.37f, -0.36f)
                curveToRelative(-0.36f, -0.62f, -1.02f, -1f, -1.73f, -1f)
                curveToRelative(-1.1f, 0f, -2f, 0.9f, -2f, 2f)
                curveToRelative(0f, 0.85f, 0.54f, 1.6f, 1.33f, 1.89f)
                curveToRelative(0.52f, 0.18f, 0.79f, 0.76f, 0.61f, 1.28f)
                close()
                moveTo(20.5f, 8f)
                curveToRelative(0f, 1.69f, -1.07f, 3.21f, -2.67f, 3.77f)
                curveToRelative(-0.11f, 0.04f, -0.22f, 0.06f, -0.33f, 0.06f)
                curveToRelative(-0.41f, 0f, -0.8f, -0.26f, -0.94f, -0.67f)
                curveToRelative(-0.18f, -0.52f, 0.09f, -1.09f, 0.61f, -1.28f)
                curveToRelative(0.8f, -0.28f, 1.33f, -1.04f, 1.33f, -1.89f)
                curveToRelative(0f, -1.1f, -0.9f, -2f, -2f, -2f)
                curveToRelative(-0.71f, 0f, -1.38f, 0.38f, -1.73f, 1f)
                curveToRelative(-0.28f, 0.48f, -0.89f, 0.64f, -1.37f, 0.36f)
                curveToRelative(-0.48f, -0.28f, -0.64f, -0.89f, -0.36f, -1.37f)
                curveToRelative(0.71f, -1.23f, 2.04f, -2f, 3.46f, -2f)
                curveToRelative(2.21f, 0f, 4f, 1.79f, 4f, 4f)
                close()
                moveTo(15.77f, 14.67f)
                curveToRelative(0.51f, 1.45f, 0.14f, 3.08f, -0.94f, 4.16f)
                curveToRelative(-0.76f, 0.76f, -1.76f, 1.17f, -2.83f, 1.17f)
                reflectiveCurveToRelative(-2.07f, -0.42f, -2.83f, -1.17f)
                curveToRelative(-0.91f, -0.91f, -1.33f, -2.22f, -1.12f, -3.49f)
                curveToRelative(0.04f, -0.23f, 0.1f, -0.45f, 0.17f, -0.67f)
                curveToRelative(0.18f, -0.52f, 0.75f, -0.79f, 1.28f, -0.61f)
                curveToRelative(0.52f, 0.18f, 0.79f, 0.76f, 0.61f, 1.28f)
                curveToRelative(-0.04f, 0.11f, -0.07f, 0.22f, -0.09f, 0.33f)
                curveToRelative(-0.11f, 0.64f, 0.1f, 1.29f, 0.56f, 1.75f)
                curveToRelative(0.76f, 0.76f, 2.07f, 0.76f, 2.83f, 0f)
                curveToRelative(0.54f, -0.54f, 0.73f, -1.36f, 0.47f, -2.08f)
                curveToRelative(-0.18f, -0.52f, 0.09f, -1.09f, 0.61f, -1.27f)
                curveToRelative(0.52f, -0.18f, 1.09f, 0.09f, 1.28f, 0.61f)
                close()
            }
        }.also { _FluxCapacitor = it}
