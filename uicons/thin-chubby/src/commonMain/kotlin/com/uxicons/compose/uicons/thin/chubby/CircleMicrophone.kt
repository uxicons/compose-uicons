package com.uxicons.compose.uicons.thin.chubby

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _CircleMicrophone: ImageVector? = null

val Icons.Tc.CircleMicrophone: ImageVector
    get() = _CircleMicrophone ?: UXIcon(name = "CircleMicrophone") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(17.99f, 11.52f)
                curveToRelative(-0.14f, 3.67f, -1.8f, 5.31f, -5.49f, 5.45f)
                verticalLineToRelative(1.53f)
                curveToRelative(0f, 0.28f, -0.22f, 0.5f, -0.5f, 0.5f)
                reflectiveCurveToRelative(-0.5f, -0.22f, -0.5f, -0.5f)
                verticalLineToRelative(-1.53f)
                curveToRelative(-3.69f, -0.14f, -5.35f, -1.78f, -5.49f, -5.45f)
                curveToRelative(-0.01f, -0.28f, 0.2f, -0.51f, 0.48f, -0.52f)
                curveToRelative(0.27f, -0.0f, 0.51f, 0.21f, 0.52f, 0.48f)
                curveToRelative(0.12f, 3.29f, 1.48f, 4.52f, 4.99f, 4.52f)
                reflectiveCurveToRelative(4.87f, -1.23f, 4.99f, -4.52f)
                curveToRelative(0.01f, -0.28f, 0.25f, -0.48f, 0.52f, -0.48f)
                curveToRelative(0.28f, 0.01f, 0.49f, 0.24f, 0.48f, 0.52f)
                close()
                moveTo(23f, 12f)
                curveToRelative(0f, 7.81f, -3.19f, 11f, -11f, 11f)
                reflectiveCurveToRelative(-11f, -3.19f, -11f, -11f)
                reflectiveCurveToRelative(3.19f, -11f, 11f, -11f)
                reflectiveCurveToRelative(11f, 3.19f, 11f, 11f)
                close()
                moveTo(22f, 12f)
                curveToRelative(0f, -7.2f, -2.8f, -10f, -10f, -10f)
                reflectiveCurveToRelative(-10f, 2.8f, -10f, 10f)
                reflectiveCurveToRelative(2.8f, 10f, 10f, 10f)
                reflectiveCurveToRelative(10f, -2.8f, 10f, -10f)
                close()
                moveTo(9.5f, 11.5f)
                verticalLineToRelative(-4f)
                curveToRelative(0f, -1.71f, 0.79f, -2.5f, 2.5f, -2.5f)
                reflectiveCurveToRelative(2.5f, 0.79f, 2.5f, 2.5f)
                verticalLineToRelative(4f)
                curveToRelative(0f, 1.71f, -0.79f, 2.5f, -2.5f, 2.5f)
                reflectiveCurveToRelative(-2.5f, -0.79f, -2.5f, -2.5f)
                close()
                moveTo(10.5f, 11.5f)
                curveToRelative(0f, 1.15f, 0.35f, 1.5f, 1.5f, 1.5f)
                reflectiveCurveToRelative(1.5f, -0.35f, 1.5f, -1.5f)
                verticalLineToRelative(-4f)
                curveToRelative(0f, -1.15f, -0.35f, -1.5f, -1.5f, -1.5f)
                reflectiveCurveToRelative(-1.5f, 0.35f, -1.5f, 1.5f)
                close()
            }
        }.also { _CircleMicrophone = it}
