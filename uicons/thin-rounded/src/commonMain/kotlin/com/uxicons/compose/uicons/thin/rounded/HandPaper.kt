package com.uxicons.compose.uicons.thin.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _HandPaper: ImageVector? = null

val Icons.Tr.HandPaper: ImageVector
    get() = _HandPaper ?: UXIcon(name = "HandPaper") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(22.12f, 3.6f)
                curveToRelative(-0.56f, -0.48f, -1.3f, -0.68f, -2.04f, -0.56f)
                curveToRelative(-0.41f, 0.07f, -0.77f, 0.24f, -1.08f, 0.49f)
                verticalLineToRelative(-0.03f)
                curveToRelative(0f, -1.38f, -1.12f, -2.5f, -2.5f, -2.5f)
                curveToRelative(-0.64f, 0f, -1.22f, 0.25f, -1.66f, 0.65f)
                curveToRelative(-0.35f, -0.96f, -1.26f, -1.65f, -2.34f, -1.65f)
                curveToRelative(-1.38f, 0f, -2.5f, 1.12f, -2.5f, 2.5f)
                verticalLineToRelative(0.03f)
                curveToRelative(-0.31f, -0.25f, -0.68f, -0.43f, -1.08f, -0.49f)
                curveToRelative(-0.73f, -0.12f, -1.48f, 0.09f, -2.04f, 0.56f)
                curveToRelative(-0.56f, 0.48f, -0.88f, 1.17f, -0.88f, 1.91f)
                verticalLineToRelative(10.6f)
                lineToRelative(-1.8f, -1.43f)
                curveToRelative(-1.01f, -0.94f, -2.59f, -0.89f, -3.53f, 0.12f)
                reflectiveCurveToRelative(-0.89f, 2.59f, 0.11f, 3.53f)
                lineToRelative(3.35f, 3.23f)
                curveToRelative(2.3f, 2.22f, 5.33f, 3.44f, 8.53f, 3.44f)
                horizontalLineToRelative(2.35f)
                curveToRelative(4.4f, 0f, 7.97f, -3.58f, 7.97f, -7.97f)
                lineTo(23f, 5.5f)
                curveToRelative(0f, -0.73f, -0.32f, -1.43f, -0.88f, -1.91f)
                close()
                moveTo(22f, 16.03f)
                curveToRelative(0f, 3.85f, -3.13f, 6.97f, -6.97f, 6.97f)
                horizontalLineToRelative(-2.35f)
                curveToRelative(-2.94f, 0f, -5.72f, -1.12f, -7.84f, -3.16f)
                lineToRelative(-3.36f, -3.24f)
                curveToRelative(-0.6f, -0.57f, -0.64f, -1.52f, -0.07f, -2.12f)
                curveToRelative(0.27f, -0.29f, 0.64f, -0.46f, 1.04f, -0.47f)
                curveToRelative(0.41f, -0.01f, 0.78f, 0.13f, 1.1f, 0.43f)
                lineToRelative(2.64f, 2.1f)
                curveToRelative(0.15f, 0.12f, 0.36f, 0.14f, 0.53f, 0.06f)
                curveToRelative(0.17f, -0.08f, 0.28f, -0.26f, 0.28f, -0.45f)
                lineTo(7f, 4.5f)
                curveToRelative(0f, -0.44f, 0.19f, -0.86f, 0.53f, -1.14f)
                curveToRelative(0.34f, -0.29f, 0.78f, -0.41f, 1.23f, -0.34f)
                curveToRelative(0.71f, 0.12f, 1.25f, 0.8f, 1.25f, 1.59f)
                verticalLineToRelative(5.89f)
                curveToRelative(0f, 0.28f, 0.22f, 0.5f, 0.5f, 0.5f)
                reflectiveCurveToRelative(0.5f, -0.22f, 0.5f, -0.5f)
                lineTo(11.0f, 2.5f)
                curveToRelative(0f, -0.83f, 0.67f, -1.5f, 1.5f, -1.5f)
                reflectiveCurveToRelative(1.5f, 0.67f, 1.5f, 1.5f)
                verticalLineToRelative(8f)
                curveToRelative(0f, 0.28f, 0.22f, 0.5f, 0.5f, 0.5f)
                reflectiveCurveToRelative(0.5f, -0.22f, 0.5f, -0.5f)
                lineTo(15.0f, 3.5f)
                curveToRelative(0f, -0.83f, 0.67f, -1.5f, 1.5f, -1.5f)
                reflectiveCurveToRelative(1.5f, 0.67f, 1.5f, 1.5f)
                verticalLineToRelative(7f)
                curveToRelative(0f, 0.28f, 0.22f, 0.5f, 0.5f, 0.5f)
                reflectiveCurveToRelative(0.5f, -0.22f, 0.5f, -0.5f)
                verticalLineToRelative(-4.89f)
                curveToRelative(0f, -0.79f, 0.54f, -1.47f, 1.24f, -1.59f)
                curveToRelative(0.45f, -0.07f, 0.89f, 0.05f, 1.23f, 0.34f)
                curveToRelative(0.34f, 0.29f, 0.53f, 0.7f, 0.53f, 1.14f)
                verticalLineToRelative(10.53f)
                close()
            }
        }.also { _HandPaper = it}
