package com.uxicons.compose.uicons.regular.straight

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _Logo: ImageVector? = null

val Icons.Rs.Logo: ImageVector
    get() = _Logo ?: UXIcon(name = "Logo") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(10f, 2f)
                curveToRelative(0f, -1.1f, 0.9f, -2f, 2f, -2f)
                reflectiveCurveToRelative(2f, 0.9f, 2f, 2f)
                reflectiveCurveToRelative(-0.9f, 2f, -2f, 2f)
                reflectiveCurveToRelative(-2f, -0.9f, -2f, -2f)
                close()
                moveTo(12f, 20f)
                curveToRelative(-1.1f, 0f, -2f, 0.9f, -2f, 2f)
                reflectiveCurveToRelative(0.9f, 2f, 2f, 2f)
                reflectiveCurveToRelative(2f, -0.9f, 2f, -2f)
                reflectiveCurveToRelative(-0.9f, -2f, -2f, -2f)
                close()
                moveTo(9.5f, 9.6f)
                curveToRelative(-0.22f, 0f, -0.4f, 0.18f, -0.4f, 0.4f)
                verticalLineToRelative(4f)
                curveToRelative(0f, 0.22f, 0.18f, 0.4f, 0.4f, 0.4f)
                reflectiveCurveToRelative(0.4f, -0.18f, 0.4f, -0.4f)
                verticalLineToRelative(-4f)
                curveToRelative(0f, -0.22f, -0.18f, -0.4f, -0.4f, -0.4f)
                close()
                moveTo(24f, 3f)
                horizontalLineToRelative(-5.7f)
                curveToRelative(2.83f, 1.99f, 4.7f, 5.28f, 4.7f, 9f)
                reflectiveCurveToRelative(-1.86f, 7.01f, -4.7f, 9f)
                horizontalLineToRelative(5.7f)
                verticalLineToRelative(2f)
                horizontalLineToRelative(-8.14f)
                curveToRelative(0.08f, -0.32f, 0.14f, -0.65f, 0.14f, -1f)
                curveToRelative(0f, -0.63f, -0.16f, -1.22f, -0.42f, -1.75f)
                curveToRelative(1.2f, -0.52f, 2.26f, -1.29f, 3.12f, -2.25f)
                horizontalLineToRelative(-13.4f)
                curveToRelative(0.86f, 0.96f, 1.92f, 1.73f, 3.12f, 2.25f)
                curveToRelative(-0.26f, 0.53f, -0.42f, 1.12f, -0.42f, 1.75f)
                curveToRelative(0f, 0.35f, 0.06f, 0.68f, 0.14f, 1f)
                horizontalLineToRelative(-8.14f)
                verticalLineToRelative(-2f)
                horizontalLineToRelative(5.7f)
                curveToRelative(-2.83f, -1.99f, -4.7f, -5.28f, -4.7f, -9f)
                reflectiveCurveToRelative(1.86f, -7.01f, 4.7f, -9f)
                horizontalLineToRelative(-5.7f)
                verticalLineToRelative(-2f)
                horizontalLineToRelative(8.14f)
                curveToRelative(-0.08f, 0.32f, -0.14f, 0.65f, -0.14f, 1f)
                curveToRelative(0f, 0.63f, 0.16f, 1.22f, 0.42f, 1.75f)
                curveToRelative(-1.2f, 0.52f, -2.26f, 1.29f, -3.12f, 2.25f)
                horizontalLineToRelative(13.4f)
                curveToRelative(-0.86f, -0.96f, -1.92f, -1.73f, -3.12f, -2.25f)
                curveToRelative(0.26f, -0.53f, 0.42f, -1.12f, 0.42f, -1.75f)
                curveToRelative(0f, -0.35f, -0.06f, -0.68f, -0.14f, -1f)
                horizontalLineToRelative(8.14f)
                close()
                moveTo(7f, 14.4f)
                horizontalLineToRelative(-1.4f)
                verticalLineToRelative(-6.4f)
                horizontalLineToRelative(-1.6f)
                verticalLineToRelative(8f)
                horizontalLineToRelative(3f)
                close()
                moveTo(11.5f, 10f)
                curveToRelative(0f, -1.1f, -0.9f, -2f, -2f, -2f)
                reflectiveCurveToRelative(-2f, 0.9f, -2f, 2f)
                verticalLineToRelative(4f)
                curveToRelative(0f, 1.1f, 0.9f, 2f, 2f, 2f)
                reflectiveCurveToRelative(2f, -0.9f, 2f, -2f)
                close()
                moveTo(16f, 9.96f)
                curveToRelative(0f, -1.1f, -0.9f, -2f, -2f, -2f)
                reflectiveCurveToRelative(-2f, 0.9f, -2f, 2f)
                verticalLineToRelative(4f)
                curveToRelative(0f, 1.1f, 0.9f, 2f, 2f, 2f)
                reflectiveCurveToRelative(2f, -0.9f, 2f, -2f)
                verticalLineToRelative(-2f)
                horizontalLineToRelative(-2f)
                verticalLineToRelative(1.5f)
                horizontalLineToRelative(0.4f)
                verticalLineToRelative(0.5f)
                curveToRelative(0f, 0.22f, -0.18f, 0.4f, -0.4f, 0.4f)
                reflectiveCurveToRelative(-0.4f, -0.18f, -0.4f, -0.4f)
                verticalLineToRelative(-4f)
                curveToRelative(0f, -0.22f, 0.18f, -0.4f, 0.4f, -0.4f)
                reflectiveCurveToRelative(0.4f, 0.18f, 0.4f, 0.4f)
                verticalLineToRelative(0.5f)
                horizontalLineToRelative(1.6f)
                close()
                moveTo(18.5f, 8f)
                curveToRelative(-1.1f, 0f, -2f, 0.9f, -2f, 2f)
                verticalLineToRelative(4f)
                curveToRelative(0f, 1.1f, 0.9f, 2f, 2f, 2f)
                reflectiveCurveToRelative(2f, -0.9f, 2f, -2f)
                verticalLineToRelative(-4f)
                curveToRelative(0f, -1.1f, -0.9f, -2f, -2f, -2f)
                close()
                moveTo(18.1f, 10f)
                verticalLineToRelative(4f)
                curveToRelative(0f, 0.22f, 0.18f, 0.4f, 0.4f, 0.4f)
                reflectiveCurveToRelative(0.4f, -0.18f, 0.4f, -0.4f)
                verticalLineToRelative(-4f)
                curveToRelative(0f, -0.22f, -0.18f, -0.4f, -0.4f, -0.4f)
                reflectiveCurveToRelative(-0.4f, 0.18f, -0.4f, 0.4f)
                close()
            }
        }.also { _Logo = it}
