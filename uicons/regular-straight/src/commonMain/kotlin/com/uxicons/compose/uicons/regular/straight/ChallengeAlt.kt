package com.uxicons.compose.uicons.regular.straight

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _ChallengeAlt: ImageVector? = null

val Icons.Rs.ChallengeAlt: ImageVector
    get() = _ChallengeAlt ?: UXIcon(name = "ChallengeAlt") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(1.5f, 3.5f)
                curveToRelative(0f, -1.38f, 1.12f, -2.5f, 2.5f, -2.5f)
                reflectiveCurveToRelative(2.5f, 1.12f, 2.5f, 2.5f)
                reflectiveCurveToRelative(-1.12f, 2.5f, -2.5f, 2.5f)
                reflectiveCurveToRelative(-2.5f, -1.12f, -2.5f, -2.5f)
                close()
                moveTo(12.29f, 8.1f)
                lineToRelative(1.4f, -0.8f)
                curveToRelative(-0.12f, -0.41f, -0.19f, -0.85f, -0.19f, -1.3f)
                reflectiveCurveToRelative(0.07f, -0.89f, 0.19f, -1.3f)
                lineToRelative(-1.4f, -0.8f)
                lineToRelative(0.99f, -1.74f)
                lineToRelative(1.4f, 0.8f)
                curveToRelative(0.61f, -0.66f, 1.41f, -1.14f, 2.31f, -1.35f)
                lineTo(17.0f, 0f)
                horizontalLineToRelative(2f)
                verticalLineToRelative(1.61f)
                curveToRelative(0.91f, 0.21f, 1.71f, 0.69f, 2.31f, 1.35f)
                lineToRelative(1.4f, -0.8f)
                lineToRelative(0.99f, 1.74f)
                lineToRelative(-1.4f, 0.8f)
                curveToRelative(0.12f, 0.41f, 0.19f, 0.85f, 0.19f, 1.3f)
                reflectiveCurveToRelative(-0.07f, 0.89f, -0.19f, 1.3f)
                lineToRelative(1.4f, 0.8f)
                lineToRelative(-0.99f, 1.74f)
                lineToRelative(-1.4f, -0.8f)
                curveToRelative(-0.61f, 0.66f, -1.41f, 1.14f, -2.31f, 1.35f)
                verticalLineToRelative(1.61f)
                horizontalLineToRelative(-2f)
                verticalLineToRelative(-1.61f)
                curveToRelative(-0.91f, -0.21f, -1.71f, -0.69f, -2.31f, -1.35f)
                lineToRelative(-1.4f, 0.8f)
                lineToRelative(-0.99f, -1.74f)
                close()
                moveTo(15.5f, 6f)
                curveToRelative(0f, 1.38f, 1.12f, 2.5f, 2.5f, 2.5f)
                reflectiveCurveToRelative(2.5f, -1.12f, 2.5f, -2.5f)
                reflectiveCurveToRelative(-1.12f, -2.5f, -2.5f, -2.5f)
                reflectiveCurveToRelative(-2.5f, 1.12f, -2.5f, 2.5f)
                close()
                moveTo(3f, 17f)
                verticalLineToRelative(3f)
                lineToRelative(-1.0f, 4f)
                lineTo(-0.07f, 24f)
                lineToRelative(1.1f, -4.38f)
                lineToRelative(-0.02f, -2.62f)
                horizontalLineToRelative(-0.01f)
                verticalLineToRelative(-7f)
                curveToRelative(0f, -1.65f, 1.35f, -3f, 3f, -3f)
                horizontalLineToRelative(4.59f)
                lineToRelative(2.21f, -2.21f)
                lineToRelative(1.41f, 1.41f)
                lineToRelative(-2.79f, 2.79f)
                horizontalLineToRelative(-2.41f)
                verticalLineToRelative(6f)
                horizontalLineToRelative(4f)
                verticalLineToRelative(3.73f)
                lineToRelative(-2f, 1.39f)
                verticalLineToRelative(-3.11f)
                lineTo(3f, 17.0f)
                close()
                moveTo(5f, 9f)
                horizontalLineToRelative(-1f)
                curveToRelative(-0.55f, 0f, -1f, 0.45f, -1f, 1f)
                verticalLineToRelative(5f)
                horizontalLineToRelative(2f)
                verticalLineToRelative(-6f)
                close()
                moveTo(6.91f, 24f)
                horizontalLineToRelative(3.27f)
                lineToRelative(13.82f, -9.59f)
                verticalLineToRelative(-2.27f)
                lineTo(6.91f, 24f)
                close()
            }
        }.also { _ChallengeAlt = it}
