package com.uxicons.compose.uicons.regular.straight

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import com.uxicons.compose.uicons.Icons
import com.uxicons.compose.uicons.core.UXIcon

private var _UserGroupCrown: ImageVector? = null

val Icons.Rs.UserGroupCrown: ImageVector
    get() = _UserGroupCrown ?: UXIcon(name = "UserGroupCrown") {
            path(fill = SolidColor(Color.Black)) {
                moveToRelative(15.03f, 19.5f)
                lineToRelative(-0.03f, 4.51f)
                lineToRelative(-2f, -0.01f)
                lineToRelative(0.03f, -4.5f)
                curveToRelative(0f, -1.37f, -1.12f, -2.49f, -2.5f, -2.49f)
                lineToRelative(-6.03f, -0.02f)
                curveToRelative(-1.34f, -0.03f, -2.53f, 1.17f, -2.5f, 2.52f)
                verticalLineToRelative(4.48f)
                horizontalLineToRelative(-2f)
                verticalLineToRelative(-4.48f)
                curveToRelative(-0.07f, -2.42f, 2.07f, -4.57f, 4.5f, -4.52f)
                lineToRelative(6.03f, 0.02f)
                curveToRelative(2.48f, 0f, 4.5f, 2.02f, 4.5f, 4.5f)
                close()
                moveTo(19.5f, 11f)
                horizontalLineToRelative(-5.47f)
                verticalLineToRelative(2f)
                horizontalLineToRelative(5.47f)
                curveToRelative(1.38f, 0f, 2.5f, 1.12f, 2.5f, 2.49f)
                lineToRelative(-0.03f, 4.5f)
                lineToRelative(2f, 0.01f)
                lineToRelative(0.03f, -4.51f)
                curveToRelative(0f, -2.48f, -2.02f, -4.5f, -4.5f, -4.5f)
                close()
                moveTo(13f, 4.5f)
                curveToRelative(0f, -2.48f, 2.02f, -4.5f, 4.5f, -4.5f)
                reflectiveCurveToRelative(4.5f, 2.02f, 4.5f, 4.5f)
                reflectiveCurveToRelative(-2.02f, 4.5f, -4.5f, 4.5f)
                reflectiveCurveToRelative(-4.5f, -2.02f, -4.5f, -4.5f)
                close()
                moveTo(15f, 4.5f)
                curveToRelative(0f, 1.38f, 1.12f, 2.5f, 2.5f, 2.5f)
                reflectiveCurveToRelative(2.5f, -1.12f, 2.5f, -2.5f)
                reflectiveCurveToRelative(-1.12f, -2.5f, -2.5f, -2.5f)
                reflectiveCurveToRelative(-2.5f, 1.12f, -2.5f, 2.5f)
                close()
                moveTo(3f, 8.5f)
                verticalLineToRelative(-6.5f)
                lineToRelative(2.25f, 2.51f)
                lineToRelative(2.25f, -2.51f)
                lineToRelative(2.25f, 2.51f)
                lineToRelative(2.25f, -2.51f)
                verticalLineToRelative(6.5f)
                curveToRelative(0f, 2.48f, -2.02f, 4.5f, -4.5f, 4.5f)
                reflectiveCurveToRelative(-4.5f, -2.02f, -4.5f, -4.5f)
                close()
                moveTo(5f, 8f)
                verticalLineToRelative(0.5f)
                curveToRelative(0f, 1.38f, 1.12f, 2.5f, 2.5f, 2.5f)
                reflectiveCurveToRelative(2.5f, -1.12f, 2.5f, -2.5f)
                verticalLineToRelative(-0.5f)
                close()
            }
        }.also { _UserGroupCrown = it}
